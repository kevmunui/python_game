    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if type(insect) == QueenAnt and insect.count == 1:
                return
            elif insect.container and insect.ant != None:
                self.ant=insect.ant
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        count = 0 #to keep track of places when moving
        while check_place is not None and check_place != hive: #exist when completely outside the range!
            if count >= self.min_range and count <= self.max_range: #if you are in the range, throw! 
                return random_or_none(check_place.bees) 
            count += 1 
            check_place = check_place.entrance #move to the next place
        return None # didn't find any nearest bee
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        "*** YOUR CODE HERE ***"
        if amount >= self.armor:#checking whether the fireant is close to dying
            for bees in list(self.place.bees):
                bees.reduce_armor(self.damage)
        return Ant.reduce_armor(self, amount)
...
class HungryAnt(Ant):
    ...
    def __init__(self):#count the number of turns it has left to digest
        Ant.__init__(self)
        "*** YOUR CODE HERE ***"
        self.digesting = 0
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    food_cost = 6
    doubles = []
    queen_place_initialized = False
    
    def __init__(self):
        "*** YOUR CODE HERE ***"
        ScubaThrower.__init__(self, 1)
        QueenAnt.number_of_queens +=1
        self.count = self.number_of_queens
    ...
    def action(self, colony):
        ...
        """
        "*** YOUR CODE HERE ***"
        #kill a queen thats an imposter
        if not QueenAnt.queen_place_initialized:
            colony.queen = QueenPlace(colony.queen, self.place)
            QueenAnt.queen_place_initialized = True
        ...
        forward_place = self.place
        backward_place = self.place.entrance

        def double_damage_for_place(action_place):
            ant = action_place.ant
            if type(ant) == QueenAnt:
                return
            if ant and ant not in self.doubles:

                if type(ant) == BodyguardAnt:
                    inner_ant = ant.ant 
                    if inner_ant and inner_ant not in self.doubles and type(inner_ant) != QueenAnt:
                        self.doubles.append(inner_ant)
                        inner_ant.damage = inner_ant.damage * 2
                self.doubles.append(ant)
                ant.damage = ant.damage * 2

        while forward_place != None:
            double_damage_for_place(forward_place)
            forward_place = forward_place.exit

        while backward_place != None:
            double_damage_for_place(backward_place)
            backward_place = backward_place.entrance
